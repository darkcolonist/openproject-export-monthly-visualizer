...
export function SupabaseModal() {
    ...
    const handleConnect = async () => {
        ...
        try {
            const count = await fetchSupabaseCount(url, key);
            setRowCount(count);
            setIsConnected(true);
        } catch (error) {
                    ...
                    ${isConnected && html`
                        <div class="space-y-5 animate-in fade-in slide-in-from-top-2 duration-300">
                            <div class="flex items-center justify-between px-1">
                                <span class="text-[10px] ${rowCount >= 1000 ? 'text-red-400' : 'text-green-400'} font-medium">Data found:</span>
                                ...
                                <div class="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4">
                                    <div class="flex gap-3">
                                        ...
                                        <p class="text-[10px] text-amber-200 leading-relaxed italic">
                                            Warning: Found ${rowCount.toLocaleString()} rows. Only 1,000 rows will be fetched. Consider narrowing your date range by setting up date filters in the legacy version first.
                                        </p>
                                    </div>
                                </div>
                            `}
